# Apex Middle East Partnership Structure - Tom Brooks

**Status:** Draft - needs legal review
**Last Updated:** 2026-01-10

---

## Corporate Structure

```
┌─────────────────────────────────────┐
│   APEX GLOBAL HOLDINGS (Richard)    │
│   - Owns all IP, designs, trademarks│
│   - 100% Richard Foulkes            │
│   - Licenses IP to regional entities│
└─────────────────┬───────────────────┘
                  │
                  │ IP License
                  │
┌─────────────────▼───────────────────┐
│      APEX ENCLOSURES ME LLC         │
│      (Dubai/UAE Entity)             │
│      - Richard: 50%                 │
│      - Tom Brooks: 50%              │
│      - Operates in GCC/Middle East  │
└─────────────────────────────────────┘
```

---

## Why This Structure

| Benefit | Explanation |
|---------|-------------|
| **IP Protection** | Global IP stays with Richard regardless of ME partnership |
| **Regional Flexibility** | ME entity can have local partners without diluting global rights |
| **Exit Protection** | If ME partnership ends, IP returns to holding company |
| **Future Regions** | Can create US entity (with Scott) under same holding company |
| **Licensing Revenue** | Holding company can charge royalty to ME entity (tax efficient) |

---

## Apex Global Holdings (Richard 100%)

**Purpose:** Own and protect all intellectual property

**Assets:**
- Product designs (ATE Series, APE Series)
- Engineering specifications
- Trademarks (Apex Enclosures)
- Manufacturing know-how
- Future product development

**Location:** UAE (RAKICC, ADGM, or DMCC free zone) or BVI/Cayman

**Richard's Rights:**
- 100% ownership
- Full control of IP licensing terms
- Can license to multiple regional entities
- Can revoke license if terms breached

---

## Apex Enclosures ME LLC (Richard 50%, Tom 50%)

**Purpose:** Operate Middle East business

**Territory:** UAE, Saudi Arabia, GCC, broader Middle East

**Responsibilities:**
- Sales and marketing in ME
- Customer relationships
- Local manufacturing partnerships (Eurotech)
- Distribution and logistics
- After-sales service

**Pays to Holding Company:**
- IP license fee (e.g., 5-10% of revenue or fixed annual fee)
- This protects IP value and provides tax-efficient extraction

---

## Buy-Sell Agreement (CRITICAL)

### Trigger Events

| Event | Buyback Triggered |
|-------|-------------------|
| Death | Yes - automatic |
| Permanent disability | Yes - automatic |
| Voluntary exit | Yes - 90 days notice |
| Termination for cause | Yes - immediate |
| Bankruptcy | Yes - automatic |
| Divorce (if shares at risk) | Right of first refusal |

### Buyback Terms

**On Death of Either Partner:**

```
1. Surviving partner MUST buy deceased's shares
2. Deceased's estate MUST sell (no option to keep)
3. Price: Fair Market Value (see valuation below)
4. Payment: Within 90 days of death
5. Funding: Key-person life insurance on both partners
```

### Valuation Method

**Option A - Formula Based (Recommended):**
```
Fair Market Value = Higher of:
  (a) 3x average net profit (last 3 years), OR
  (b) 1x revenue (last 12 months), OR
  (c) Book value of net assets

Minimum floor: Total capital contributed
```

**Option B - Independent Appraisal:**
- Each party appoints one valuer
- If valuations differ >15%, third valuer decides
- Cost shared equally
- Takes longer but more accurate

**Annual Review:**
- Partners review and agree valuation annually
- Signed statement of agreed value
- Simplifies process if trigger event occurs

### Life Insurance Funding

| Policy | Owner | Insured | Beneficiary | Purpose |
|--------|-------|---------|-------------|---------|
| Policy A | Company | Richard | Company | Fund buyback if Richard dies |
| Policy B | Company | Tom | Company | Fund buyback if Tom dies |

**Coverage Amount:** Equal to estimated 50% share value (review annually)
**Type:** Term life (cheaper) or whole life (builds value)

---

## IP License Agreement

**From:** Apex Global Holdings (Richard)
**To:** Apex Enclosures ME LLC

### Key Terms

| Term | Details |
|------|---------|
| **Scope** | Non-exclusive license for ME territory |
| **Products** | ATE Series, APE Series, future products |
| **Fee** | 5% of net revenue OR $X fixed annual minimum |
| **Term** | Renewable annually |
| **Termination** | 90 days notice, or immediate if ME entity breaches |

### Richard's Reserved Rights

- License to other regions (US, EU, APAC)
- Modify or improve designs
- Create new products
- Revoke license if ME entity:
  - Fails to pay license fees
  - Damages brand reputation
  - Breaches quality standards
  - Becomes insolvent

---

## Decision Rights

### Day-to-Day Operations (Either Partner)
- Sales under $50,000
- Hiring staff under manager level
- Marketing spend within budget
- Routine supplier orders

### Major Decisions (Both Must Agree)
- Sales over $50,000
- Hiring senior management
- Taking on debt
- New product launches
- Changing suppliers (e.g., moving from Eurotech)
- Any IP-related decisions
- Distributions over $X

### Deadlock Resolution
If partners cannot agree on major decision:
1. 14-day cooling off period
2. Mediation (DIFC or ADCCAC)
3. If still deadlocked: Either partner can trigger buyout
   - Offering partner names price
   - Other partner must buy at that price OR sell at that price
   - "Shotgun clause" - keeps offers fair

---

## Exit Scenarios

### Scenario 1: Tom Dies
1. Life insurance pays out to company
2. Company buys Tom's 50% at FMV
3. Money goes to Tom's estate
4. Richard owns 100% of ME entity
5. IP remains with holding company (unchanged)

### Scenario 2: Richard Dies
1. Life insurance pays out to company
2. Company buys Richard's 50% at FMV
3. Money goes to Richard's estate
4. Tom owns 100% of ME entity
5. **IP License continues AUTOMATICALLY** (critical - irrevocable)
6. Tom can continue operating under license indefinitely
7. Richard's heirs inherit holding company + IP

**CRITICAL - Tom's Protection from Richard's Heirs:**
- IP License is **IRREVOCABLE** for ME territory as long as Tom operates
- Richard's heirs CANNOT:
  - Revoke or terminate Tom's IP license
  - Raise license fees beyond agreed escalation
  - Block Tom's ME operations
  - Interfere with Tom's business decisions
- Heirs receive license fee income (5% of revenue) but NO control
- License continues for life of the ME business or until Tom chooses to exit
- If heirs want to sell holding company, Tom has RIGHT OF FIRST REFUSAL

### Scenario 3: Tom Wants Out
1. Tom gives 90 days notice
2. Richard has right of first refusal to buy at FMV
3. If Richard declines, Tom can sell to approved third party
4. Richard retains veto on any buyer

### Scenario 4: Partnership Breakdown
1. Shotgun clause triggered
2. One partner offers to buy other out at $X
3. Other partner must accept OR buy at same price
4. Loser exits, winner takes 100%
5. IP stays with holding company regardless

---

## Documents Needed

| Document | Purpose | Est. Cost |
|----------|---------|-----------|
| Holding Company Formation | Protect IP | $2,000-5,000 |
| ME LLC Formation | Operating entity | $1,500-3,000 |
| Shareholders Agreement | 50/50 terms, decision rights | $2,000-4,000 |
| Buy-Sell Agreement | Death/exit buyback | Included above |
| IP License Agreement | Holding → ME entity | $1,000-2,000 |
| Life Insurance (x2) | Fund buybacks | ~$50-100/month each |

**Total Setup:** ~$8,000-15,000 legal + ongoing insurance

---

## Next Steps

- [ ] Discuss structure with Tom
- [ ] Agree on valuation method
- [ ] Agree on license fee %
- [ ] Get life insurance quotes (both partners)
- [ ] Engage UAE corporate lawyer
- [ ] Form holding company
- [ ] Form ME operating company
- [ ] Execute all agreements

---

## Notes

- This is a DRAFT framework - needs proper legal review
- UAE law governs ME entity
- Consider ADGM or DIFC for holding company (English common law)
- Tax advice needed on optimal holding company jurisdiction
- Tom should have independent legal advice before signing

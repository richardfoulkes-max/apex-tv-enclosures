...
(function() {
    ...
    if (isProtected) {
        const auth = JSON.parse(localStorage.getItem('apexAuth') || '{}');
        const now = Date.now();
        const thirtyDays = 30 * 24 * 60 * 60 * 1000;

        if (!auth.token || !auth.timestamp || (now - auth.timestamp) > thirtyDays) {
            // Show login gate
            document.documentElement.style.display = 'none';

            document.addEventListener('DOMContentLoaded', function() {
                document.body.innerHTML = `
                    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
                        <div style="background:white;padding:2.5rem;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.1);max-width:360px;width:90%;">
                            <div style="text-align:center;margin-bottom:1.5rem;">
                                <div style="width:48px;height:48px;background:linear-gradient(135deg,#6366f1,#4f46e5);border-radius:12px;display:inline-flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:1.25rem;margin-bottom:1rem;">A</div>
                                <h1 style="font-size:1.25rem;color:#1e293b;margin:0;">Apex Internal</h1>
                                <p style="color:#64748b;font-size:0.875rem;margin-top:0.25rem;">Enter password to continue</p>
                            </div>
                            <form id="apexLoginForm">
                                <input type="password" id="apexPassword" placeholder="Password"
                                    style="width:100%;padding:0.75rem 1rem;border:1px solid #e2e8f0;border-radius:8px;font-size:1rem;margin-bottom:1rem;box-sizing:border-box;"
                                    autocomplete="current-password">
                                <button type="submit"
                                    style="width:100%;padding:0.75rem;background:linear-gradient(135deg,#6366f1,#4f46e5);color:white;border:none;border-radius:8px;font-size:1rem;font-weight:500;cursor:pointer;">
                                    Login
                                </button>
                                <p id="apexError" style="color:#ef4444;font-size:0.875rem;text-align:center;margin-top:1rem;display:none;">Incorrect password</p>
                            </form>
                        </div>
                    </div>
                `;
                ...
                document.getElementById('apexPassword').focus();
                document.getElementById('apexLoginForm').addEventListener('submit', function(e) {
                    e.preventDefault();
                    const pwd = document.getElementById('apexPassword').value;
                    const storedPwd = localStorage.getItem('apexAdminPassword') || 'apex2025';

                    if (pwd === storedPwd) {
                        localStorage.setItem('apexAuth', JSON.stringify({ token: 'authenticated', timestamp: Date.now() }));
                        window.location.reload();
                    } else {
                        document.getElementById('apexError').style.display = 'block';
                        document.getElementById('apexPassword').value = '';
                        document.getElementById('apexPassword').focus();
                    }
                });
            });
            return; // Stop nav.js execution
        }
    }
